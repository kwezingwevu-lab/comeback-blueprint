...
> - Sandbox facts: `fantasy.premierleague.com` and `draft.premierleague.com` are reachable through the session proxy; Playwright's Chromium is preinstalled under `/opt/pw-browsers` (never run `playwright install`); WebKit is not installed.
...
4. **Minutes model**: logistic on {starts_last3, minutes trend, flag, days since last start, competition load (Europe midweek)}; replaces the Laplace rate; scored on P(start) calibration (Brier).
5. **Player-level xG model**: `xG90 × att_team × def_opp` into the E1 shape; must beat BPS-rate in the tournament before driving anything.
...
8. **Chip solver on the real calendar**: enumerate confirmed DGW/BGW from `fixtures/`; BB value = Σ bench xp in the double; TC = 3× best single; FH = best15 in a blank minus current; solve jointly under set-1/set-2 expiry.
